...
<head>
    ...
    <title>Jewelry Shop- Cart page</title>
    <!-- Bootstrap -->
    ...
    <thead>
      ...
      <tbody>
        <tr data-ng-repeat="x in products |orderBy:sortKey:reverse| filter:search">
			<td>{{ x.Name }}</td>
			<td><input type="number" data-ng-model="x.Quantity"/></td>
			<td>{{ x.Price }}</td>
			<td>{{ x.Quantity * x.Price }}</td>
			<td class="text-center"><a href="cart/deletefromcart?id={{ x.Id }}" class="btn btn-danger btn-xs"><span class="glyphicon glyphicon-remove"></span> Del</a></td>
        </tr>
      </tbody>
      </table>
      <a href="cart/addmorepdts"><button type="button" class="btn btn-default">Continue Shopping</button></a>
      <a href="memberShip"><button type="button" class="btn btn-default">Checkout</button></a>
    </div>
</div>
<script>
